...
class ChuckNorrisPlugin(BasePlugin):
    ...
    @BasePlugin.respond_to('chuck norris')
    def chuck_norris_me(self, message_data):
        """
        chuck norris: Get the a joke about Chuck Norris (there is no gaurentee it will be good).
        """
        mwp_room_client.send_notification(self.cn.get_chuck_norris_joke())

    @BasePlugin.respond_to('joke (?P<first_name>.*) (?P<last_name>.*)')
